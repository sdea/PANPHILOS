...
public:
      ...
      SimulationParameter3D(std::string xmlFileName) {
	    ...
	    xmlFile["Geometry"]["inputCosThetaFile"].read(fileCosThetaName);
        ...
        void print() {
            ...
            pcout << "*      Input CosTheta File:   " << this->fileCosThetaName << std::endl;
...
public:
	...
	// XML input file name
	std::string fileCName, filePsiName, fileCosThetaName;
...
void writeFullVTK(std::vector<MultiBlock3D* > &fields, Box3D domain, std::string fileName, plint iter, plint num_zeros) {
    ...
    MultiScalarField3D<T>& mu = *dynamic_cast<MultiScalarField3D<T>* >(fields[2]);
