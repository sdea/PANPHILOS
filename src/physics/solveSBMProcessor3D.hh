#ifndef SOLVESBMPROCESSOR3D_HH_INCLUDED
#define SOLVESBMPROCESSOR3D_HH_INCLUDED
...
template<typename T>
void solveSBM3D<T>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D* > atomicBlocks) {
...
template<typename T>
solveSBM3D<T>*
        solveSBM3D<T>::clone() const {

    return new solveSBM3D<T>(*this);
}

 
template<typename T>
void solveSBM3D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
...
#endif // SOLVESBMPROCESSOR3D_HH_INCLUDED
