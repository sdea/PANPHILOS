#ifndef SOLVESBMPROCESSORCONTACTANGLE2D_HH_INCLUDED
#define SOLVESBMPROCESSORCONTACTANGLE2D_HH_INCLUDED

#include "solveSBMProcessorContactAngle2D.h"

template<typename T>
solveSBMContactAngle2D<T>::solveSBMContactAngle2D(T eps2_, T Q_, T invh2_, T dt_, T M_, T h_, T cosTheta_, bool SM_, bool useG_):
                    eps2(eps2_), Q(Q_), invh2(invh2_), dt(dt_), M(M_), h(h_), cosTheta(cosTheta_), SM(SM_),
		    useG(useG_)	{ }

// Override generic process method
template<typename T>
void solveSBMContactAngle2D<T>::processGenericBlocks(Box2D domain, std::vector<AtomicBlock2D* > atomicBlocks) {
     ...
     ScalarField2D<T>& theta = *dynamic_cast<ScalarField2D<T>* >(atomicBlocks[3]);
     
    // Compute displacments
     Dot2D offset_C = computeRelativeDisplacement(mu, C);
     Dot2D offset_Psi = computeRelativeDisplacement(mu, Psi);
     Dot2D offset_theta = computeRelativeDisplacement(mu, theta);
	 ...
	 plint iX_theta = iX + offset_theta.x;
	    ...
	    plint iY_theta = iY + offset_theta.y;
		...
		T dPy = dyPsi/Psi.get(iX_Psi,iY_Psi);
                ...
                mu.get(iX,iY) = dF- eps2*(dotdC_dPsiOvPsi + laplacianC) -
                                        std::sqrt(eps2)*
                                        modGPsiOvPsi*
                                        std::sqrt(2*F)*theta.get(iX_theta,iY_theta);
...
template<typename T>
solveSBMContactAngle2D<T>*
        solveSBMContactAngle2D<T>::clone() const {

    return new solveSBMContactAngle2D<T>(*this);
}

 
template<typename T>
void solveSBMContactAngle2D<T>::getTypeOfModification(std::vector<modif::ModifT>& modified) const {
...
#endif // SOLVESBMPROCESSORCONTACTANGLE2D_HH_INCLUDED
